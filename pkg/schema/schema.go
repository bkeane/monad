...
var Version = map[string]Spec{
	"1.1": &Schema_1_1{
		...
		Name: StringLabel{
			...
			Key:         "org.kaixo.monad.run.name",
			Required:    true,
		},
		Branch: StringLabel{
			Description: "Git branch string",
			Key:         "org.kaixo.monad.run.git.branch",
			Required:    true,
		},
		Sha: StringLabel{
			Description: "Git sha string",
			Key:         "org.kaixo.monad.run.git.sha",
			Required:    true,
		},
		Origin: StringLabel{
			Description: "Git origin string",
			Key:         "org.kaixo.monad.run.git.origin",
			Required:    true,
		},
		Owner: StringLabel{
			Description: "Git owner string",
			Key:         "org.kaixo.monad.run.git.owner",
			Required:    true,
		},
		Repo: StringLabel{
			Description: "Git repo string",
			Key:         "org.kaixo.monad.run.git.repo",
			Required:    true,
		},
		Dirty: BoolLabel{
			Description: "Git dirty state",
			Key:         "org.kaixo.monad.run.git.dirty",
			Required:    true,
		},
		Role: FileLabel{
			Description: "Role template file",
			Key:         "org.kaixo.monad.run.role",
			Default:     filepath.Join("defaults", "role.json.tmpl"),
			Required:    false,
		},
		Policy: FileLabel{
			Description: "Policy template file",
			Key:         "org.kaixo.monad.run.policy",
